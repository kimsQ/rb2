...
<title></title>
<style>
body {padding:0;margin:0;background:#0d0d0d;}
#_colorChart_ { BACKGROUND: #0d0d0d; COLOR: #AAA; DISPLAY: block; FONT-FAMILY: arial; FONT-SIZE: 11px; PADDING: 7px 10px 11px 10px; _PADDING-RIGHT: 0; Z-INDEX: 1; TOP: 0; LEFT: 0; POSITION: absolute; WIDTH: 199px; _width: 210px; _padding-right: 0px; }
#_colorChart_ br { CLEAR: both; MARGIN: 0; PADDING: 0;  }
#_colorChart_ select { BORDER: 1px solid #333; BACKGROUND: #FFF; POSITION: relative; TOP: 4px; }

#plugHEX { FLOAT: left; }
#plugCLOSE { FLOAT: right; MARGIN: 0 8px 3px; _MARGIN-RIGHT: 10px; COLOR: #FFF; -moz-user-select: none; -khtml-user-select: none; user-select: none; }
...
<div id="_colorChart_" style="top: 0px; left: 0px; z-index: 20;">
<div id="plugHEX">#<?php echo $_GET['color']?$_GET['color']:'F1FFCC'?></div><br>
<div id="SV" onmousedown="HSVslide('SVslide','_colorChart_',event)">
<div id="SVslide" style="TOP: -4px; LEFT: -4px;"><br /></div>
</div>
<form id="H" onmousedown="HSVslide('Hslide','_colorChart_',event)">
<div id="Hslide" style="TOP: -7px; LEFT: -8px;"><br /></div>
<div id="Hmodel"></div>
</form>
</div>
<div id="cbox" style="cursor:pointer;position:absolute;top:190px;left:10px;width:198px;height:30px;background:#<?php echo $_GET['color']?$_GET['color']:'F1FFCC'?>;" onclick="aplyCcolor();">&nbsp;</div>

<script type="text/javascript">
function aplyCcolor()
{
	parent._small_modal_close_();
}
function mkColor(v)
{
	document.getElementById('cbox').style.background = '#'+v;
	<?php if($_GET['dropf']):?>
	parent.document.getElementById('<?php echo $_GET['dropf']?>').value = '#'+v;
	<?php endif?>
	<?php if($_GET['callback']):?>
	parent.<?php echo $_GET['callback']?>('<?php echo $_GET['dropf']?>',v);
	<?php endif?>
}
loadSV(); 
updateH('<?php echo $_GET['color']?$_GET['color']:'F1FFCC'?>');

function modalSetting()
{
	parent.getId('_small_modal_body_').style.top = '30%';
	parent.getId('_small_modal_body_').style.width = '225px';
	parent.getId('_small_modal_body_').style.height = '220px';
	parent.getId('_small_modal_').style.background = '#0d0d0d';
	parent.getId('_small_modal_iframe_').style.width = '220px';
	parent.getId('_small_modal_iframe_').style.height = '230px';
}
modalSetting();
</script>
